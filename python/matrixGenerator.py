...
def save_matrix_and_vector(matrix, vector, result, filename):
    """
    Salva la matrice, il vettore e il risultato in file .npz e .txt.
    :param matrix: matrice sparsa
    :param vector: vettore denso
    ...
    np.savez(filename, matrix_data=matrix.data, matrix_indices=matrix.indices,
             matrix_indptr=matrix.indptr, vector=vector, result=result)
...
def main():
    size = 100  # Dimensione della matrice (size x size)
    ...
    for sparsity in sparsity_levels:
        for stride in strides:
            ...
            # Salva la matrice CSC, il vettore e il risultato nella cartella "csc_matrici"
            filename_csc = f"matrix_sparsity_{int(sparsity*100)}_stride_{stride}"
            filepath_csc = os.path.join(csc_folder, filename_csc)
            save_matrix_and_vector(matrix_csc, vector, result_csc, filepath_csc)
            print(f"Saved CSC: {filepath_csc}.npz, {filepath_csc}_matrix.txt, {filepath_csc}_vector.txt, {filepath_csc}_result.txt")
            
            # Salva la matrice CSR, il vettore e il risultato nella cartella "csr_matrici"
            filename_csr = f"matrix_sparsity_{int(sparsity*100)}_stride_{stride}"
            filepath_csr = os.path.join(csr_folder, filename_csr)
            save_matrix_and_vector(matrix_csr, vector, result_csr, filepath_csr)
            print(f"Saved CSR: {filepath_csr}.npz, {filepath_csr}_matrix.txt, {filepath_csr}_vector.txt, {filepath_csr}_result.txt")
