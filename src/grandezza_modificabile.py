...
def generate_mlir(m, k, n, sparsity):
    ...
    result_matrix = sparse_matrix @ dense_matrix2
    
    # Save matrices to files
    np.savetxt("sparse_matrix.txt", sparse_matrix.toarray(), fmt="%.6f")
    np.savetxt("dense_matrix.txt", dense_matrix2, fmt="%.6f")
    np.savetxt("result_matrix.txt", result_matrix, fmt="%.6f")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--m", type=int, default=200, help="Number of rows in the sparse matrix")
    parser.add_argument("--k", type=int, default=200, help="Number of columns in the sparse matrix / rows in dense matrix")
    parser.add_argument("--n", type=int, default=200, help="Number of columns in the dense matrix")
    parser.add_argument("--sparsity", type=float, default=0.5, help="Sparsity level of the matrix (0 to 1)")
    args = parser.parse_args()
    generate_mlir(args.m, args.k, args.n, args.sparsity)
